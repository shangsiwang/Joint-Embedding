...
\begin{proof} [Proof of Theorem 2.1]
 A real symmetric matrix of size $n$ has $\frac{n(n+1)}{2}$ free entries which lies in a linear space with dimensionality $\frac{n(n+1)}{2}$. As a consequence, if there exists $\frac{n(n+1)}{2}$ linearly independent rank one symmetric matrices, they form a basis for this space. It turns out that the rank one symmetric matrices generated by vectors $\{e_i\}_{i=1}^{n} \cup \{e_i+e_j\}_{i<j}$ are linearly independent, where $\{e_i\}_{i=1}^{n}$ is the standard basis for $n$-dimensional Euclidean space. Let $\{e_i\}_{i=1}^{n} \cup \{\frac{e_i+e_j}{\sqrt{2}}\}_{i<j}$ be the $h$s in MREG model. Let $A_1,A_2,..A_{2^\frac{n(n+1)}{2}}$ be all the possible adjacency matrices on $n$ vertices, and $p_i$ be the probability of observe $A_i$ under the distribution $\mathcal{F}$. The previous discussion implies that for each adjacency matrix $A_i$, there exist a vector $\lambda_i$ such that $A_i =\sum_{k} \lambda_i [k] h_k  h_k^T$. Let $F$ be a finite mixture distribution on points $\lambda_i$, that is $F = \sum p_i \mathbb{I} \{\lambda = \lambda_i\}$. Then, we have
 \[P_{\mathcal{F}}(A) = p_i = P(\lambda = \lambda_i) = P_{MREG}(A).\]
 This concludes that $\mathcal{F}$ and the MREG has the same distribution on graphs.
\end{proof}
