...
\begin{abstract}
In this paper, we propose an method to jointly embed multiple graphs into low dimensional space. The method identifies a linear subspace  and projects adjacency matrices into the subspace. The corresponding coefficients can be treated as features of graphs. We propose a random graph model and prove that our method produce estimates with small errors under simple settings.  We demonstrate through simulation studies and real data experiments that our joint embedding method leads to state of the art performance in inference on multiple graphs. 
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
random graph, feature extraction, inference on multiple graphs
\end{IEEEkeywords}}
