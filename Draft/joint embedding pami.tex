...
\begin{figure}[!htbp]
	...
	\caption{Relationship between random graph models on $1$ graph. ER is a $1$-block SBM. If a graph follows SBM with positive semi-definite edge probability matrix, it also follows RDPG model. Any SBM and RDPG graph can be represented by a $d$-dimensional MREG model with $d$ being less than or equal to the number of block or dimension of RDPG. On one graph, inhomogeneous ER and $n$-dimensional MREG are equivalent.}
	\label{fig:ven}
\end{figure}
